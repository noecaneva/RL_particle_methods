...
class swarm:
    ...
    def retpreComputeStates(self, fishes):
        ...
        if self.dim == 2:
            ...
            anglesTheta = np.arccos(dz, distances)
            #print("dz")
            #print(dz)
            #print("dist")
            #print(distances)
    ...
    def getState( self, i ):
        ...
        gausDistancesNearestNeighbours = 1./np.sqrt(2.*np.pi*(0.5*self.rAttraction)**2)*np.exp( - 0.5 * (distances[idNearestNeighbours]/(self.rAttraction*0.5))**2 )
        ...
        if self.dim == 2:
        	...
        	return np.array([ gausDistancesNearestNeighbours, anglesNearestNeighbours]).flatten()
        else:
        	...
        	return np.array([ gausDistancesNearestNeighbours, anglesPhiNearestNeighbours, anglesThetaNearestNeighbours]).flatten()
