...
e["Problem"]["Testing Batch Size"] = nfish
...
#csel = plt.cm.coolwarm(norm(rewards))
csel = plt.cm.coolwarm(rewards)

print(f"Plotting")
if args.D == 2:
    plotSwarm2DFinal(tridx, tidx, np.array(locations), np.array(directions), csel=csel, rewards=rewards)

elif args.D == 3:
    plotSwarm3DFinal(tridx, tidx, np.array(locations), np.array(directions), csel=csel, rewards=rewards)
