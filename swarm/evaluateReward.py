...
from swarm import *
colors=['darkblue','darkred','goldenrod']
...
parser.add_argument('--tidx', help='Index in trajectory.', required=True, nargs='+', type=int)
...
rfile=f"rewards_{tridx}_{tidx}.pdf"
...
for i, t in enumerate(tidx):
    ...
    sim.initFromLocationsAndDirections(locations[t,:,:], directions[t,:,:])
    ...
    #e["Solver"]["Neural Network"]["Output Layer"]["Shift"][0] = 0.
    #e["Solver"]["Neural Network"]["Output Layer"]["Transformation Mask"][0] = "NegIExp"
    ...
    plt.plot(rotations, muRewardHat, '-', color=colors[i], linewidth=2)
    ...
    if args.D == 2:
        plotSwarm2DFinal(tridx, t, np.array(locations), np.array(directions))

    elif args.D == 3:
        plotSwarm3DFinal(tridx, t, np.array(locations), np.array(directions))
